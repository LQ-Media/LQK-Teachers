  ...
  const rows = getDb()
    .prepare(
      `SELECT l.*, p.full_name AS subject_name
       ...
       -- rowid breaks the tie: two changes in the same millisecond are real
       -- (grant centre, then correct it to full), and without it the log would
       -- show them in whichever order SQLite happened to return.
       ORDER BY l.changed_at DESC, l.rowid DESC LIMIT 50`
    )
    .all()
    .map((r) => ({
      id: r.id,
