...
export async function createUser(data) {
  ...
  db.prepare(
    `INSERT INTO profiles (id, full_name, email, password_hash, role, admin_scope, primary_location, position, pay_tier, photo, must_change_password, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, 1, ?)`
  ).run(id, full_name, email, hashPassword(tempPassword), role, adminScopeFor(role, data.adminScope), primary_location || null, position || null, pay_tier, new Date().toISOString());
...
export async function updateUser(data) {
  ...
  db.prepare(
    "UPDATE profiles SET full_name = ?, email = ?, role = ?, admin_scope = ?, primary_location = ?, position = ?, pay_tier = ? WHERE id = ?"
  ).run(full_name, email, role, adminScopeFor(role, data.adminScope), primary_location || null, position || null, pay_tier, id);
