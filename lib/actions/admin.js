...
/**
 * The admin tier to store for a role.
 *
 * NULL for anyone who is not an admin, so the column never implies a tier
 * somebody does not have. For an admin it defaults to CENTRE: creating a full
 * admin should be a deliberate choice, and the safe failure for a forgotten
 * field is less access, not payroll.
 */
function adminScopeFor(role, requested) {
  if (role !== "admin") return null;
  return requested === "full" ? "full" : "centre";
}

/**
 * Replace an admin's managed branches.
 *
 * Always a full replacement inside the caller's transaction, never a merge: an
 * edit that removes a centre has to actually remove it, and a diff-based update
 * is one missed case away from leaving somebody access they were just told they
 * no longer have.
 */
function setManagedBranches(db, userId, role, scope, branches) {
  db.prepare("DELETE FROM manager_branches WHERE manager_id = ?").run(userId);
  if (role !== "admin" || scope !== "centre") return;
  const known = new Set(LOCATIONS);
  for (const b of new Set((branches || []).map(clean).filter((b) => known.has(b)))) {
...
const PROFILE_REFS = [
  ...
  ["invites", "invited_by"],
  // Peer assessment: the result outlives the assessor's account; the teacher's
  // own results cascade from profiles.
...
export async function createUser(data) {
  ...
  const is_assessor = data.is_assessor ? 1 : 0;
  ...
  db.prepare(
    `INSERT INTO profiles (id, full_name, email, password_hash, role, admin_scope, primary_location, position, pay_tier, photo, must_change_password, is_assessor, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, 1, ?, ?)`
  ).run(id, full_name, email, hashPassword(tempPassword), role, adminScopeFor(role, data.adminScope), primary_location || null, position || null, pay_tier, is_assessor, new Date().toISOString());

  setManagedBranches(db, id, role, adminScopeFor(role, data.adminScope), data.managedBranches);
...
export async function updateUser(data) {
  ...
  db.prepare(
    "UPDATE profiles SET full_name = ?, email = ?, role = ?, admin_scope = ?, primary_location = ?, position = ?, pay_tier = ?, is_assessor = ? WHERE id = ?"
  ).run(full_name, email, role, adminScopeFor(role, data.adminScope), primary_location || null, position || null, pay_tier, is_assessor, id);

  // Demoting out of admin drops the branches with it. Leaving them behind would
  // silently restore a centre admin's old reach the moment anybody promoted
  // them again.
  setManagedBranches(db, id, role, adminScopeFor(role, data.adminScope), data.managedBranches);
  rebuildLocations(db, id, branches, primary_location);
