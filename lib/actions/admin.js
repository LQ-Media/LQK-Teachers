...
export async function createUser(data) {
  ...
  db.prepare(
    `INSERT INTO profiles (id, full_name, email, password_hash, role, primary_location, position, pay_tier, photo, must_change_password, is_assessor, created_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, NULL, 1, ?, ?)`
  ).run(id, full_name, email, hashPassword(tempPassword), role, primary_location || null, position || null, pay_tier, is_assessor, new Date().toISOString());
...
export async function updateUser(data) {
  ...
  db.prepare(
    "UPDATE profiles SET full_name = ?, email = ?, role = ?, primary_location = ?, position = ?, pay_tier = ?, is_assessor = ? WHERE id = ?"
  ).run(full_name, email, role, primary_location || null, position || null, pay_tier, is_assessor, id);
  rebuildLocations(db, id, branches, primary_location);
