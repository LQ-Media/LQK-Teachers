...
// SHIFT_LOCATIONS is what may be OFFERED for a new shift; SHIFT_LOCATIONS_ALL
// adds the pre-split "Woods Square", which existing shifts still carry and
// must stay editable at.
import { SHIFT_LOCATIONS, SHIFT_LOCATIONS_ALL, expandManagedBranches } from "@/lib/hours/locations";
...
import { repeatMeaning } from "@/lib/hours/positions";
// The LIVE list, not the code one: the positions screen is editable, so a
// position Karim added this morning has to be creatable this morning.
import { positionMeaningLive } from "@/lib/hours/position-store";
...
function shape(row) {
  ...
  return {
    ...
    // The colour chosen for this position on the Positions screen, if any.
    // NULL means "derive it from the name", which is what every shift did
    // before the list became editable.
    positionColour: row.position_colour ?? null,
...
const SELECT = `
  ...
  LEFT JOIN shift_positions sp ON sp.name = s.position
...
function visibleTo(session, rows) {
  ...
  // Expanded: managing "Woods Square" means seeing all three of its rooms, and
  // the shifts still carrying the pre-split name. A plain Set of the managed
  // branches would have shown a Woods Square IT Head an empty roster.
  const set = new Set(expandManagedBranches(mine));
...
export async function editShift(data) {
  ...
  if (data.branch !== undefined) {
    const target = SHIFT_LOCATIONS_ALL.includes(clean(data.branch)) ? clean(data.branch) : null;
  ...
  const branch = SHIFT_LOCATIONS_ALL.includes(clean(data.branch)) ? clean(data.branch) : row.branch;
...
export async function cancelDate(date, branch, reason) {
  ...
  const b = SHIFT_LOCATIONS_ALL.includes(clean(branch)) ? clean(branch) : null;
 ...
 */
/**
 * The public holidays we hold, this year and next.
 *
 * Read-only, and it exists so the Settings screen can show WHAT was imported
 * rather than only offering a button that imports it. "Sync holidays" with no
 * list beside it gives somebody no way to tell a successful sync from a sync
 * that quietly fetched nothing.
 */
export async function publicHolidays() {
  await requireFullAdmin();
  const year = Number(sgToday().slice(0, 4));
  const rows = getDb()
    .prepare("SELECT date, name FROM public_holidays WHERE date >= ? AND date < ? ORDER BY date ASC")
    .all(`${year}-01-01`, `${year + 2}-01-01`)
    .map((r) => ({ date: r.date, name: r.name }));
  return { years: [year, year + 1], holidays: rows };
}
...
export async function createShifts(data) {
  ...
  const meaning = positionMeaningLive(data.position);
  if (!meaning) return { error: "Pick a position." };

  // The offered list, deliberately not SHIFT_LOCATIONS_ALL: a NEW shift at
  // Woods Square has to name which room. The old value stays valid on shifts
  // that already have it and is not a choice going forward.
