...
function shape(row) {
  ...
  return {
    ...
    // The teacher's job title, free text from the imported Sheet. Used by the
    // calendar to colour a shift by role, the way Sling colours by position.
...
const SELECT = `
  SELECT s.*, p.full_name AS teacher_name, p.pay_tier AS pay_tier, p.position AS position,
...
export async function createShift(data) {
  ...
  logShift(db, session, newId, "created");
...
export async function editShift(data) {
  ...
  if (data.teacherId !== undefined && clean(data.teacherId) && clean(data.teacherId) !== row.teacher_id) {
    ...
    // in createShift: an admin can still fix it, and the teacher is the wrong
...
export async function generateShifts(data) {
  ...
  const category = data.category === "ot" ? "ot" : "teaching";
  const branch = SHIFT_LOCATIONS.includes(clean(data.branch)) ? clean(data.branch) : null;
  const deniedBranch = refuseBranch(session, branch);
  if (deniedBranch) return { error: deniedBranch };
 ...
 *     against whoever ends up holding them — the same check createShift makes,
...
export async function splitShift(shiftId, at, firstTeacherId, secondTeacherId) {
  ...
  try {
    ...
    db.prepare(
      `INSERT INTO shifts (id, teacher_id, category, ot_reason, ot_role, branch, date, start_time, end_time,
                           starts_at, ends_at, status, ph_name, unpaid, note, batch_id, created_by, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'planned', ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      randomUUID(), secondTeacher, row.category, row.ot_reason, row.ot_role, row.branch, row.date,
      cut, row.end_time, secondHalf.startsAt, secondHalf.endsAt,
      row.ph_name, row.unpaid, note, row.batch_id, session.userId, now, now
    );
    db.exec("COMMIT");
  } catch (err) {
    db.exec("ROLLBACK");
 ...
 * createShift does, and records itself as a creation on the NEW shift rather
...
export async function duplicateShift(id, toDate) {
  ...
  try {
    db.prepare(
      ...
      copyId, row.teacher_id, row.category, row.ot_reason, row.ot_role, row.branch, date,
      row.start_time, row.end_time, span.startsAt, span.endsAt, ph, row.unpaid, row.note,
