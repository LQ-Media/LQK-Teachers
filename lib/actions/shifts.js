...
import { holidayMap } from "@/lib/hours/holidays";
...
export async function generateShifts(data) {
  ...
  const existing = db
    ...
    .all(teacherId, fromDate, addSgDays(toDate, 1))
    .map(shape);
  ...
  if (clashes.length) {
    return {
      error: `${clashes.length} of these clash with shifts this teacher already has (${clashes.slice(0, 3).join(", ")}${clashes.length > 3 ? "…" : ""}). Nothing was created.`,
      clashes,
    };
  }
  ...
  for (const r of rows) {
    try {
      insert.run(
        randomUUID(), teacherId, category, null, branch, r.date, startTime, endTime,
        r.startsAt, r.endsAt, r.phName, data.unpaid ? 1 : 0, clean(data.note) || null,
        batchId, session.userId, now, now
      );
      created += 1;
    } catch (err) {
      // The unique index caught a re-submitted form — an idempotent no-op, not
      // a failure.
  ...
  revalidatePath("/admin");
  revalidatePath("/hours");
  return { ok: true, created, duplicates, skipped, batchId };
}
