...
export async function login(prevState, formData) {
  ...
  // Stamped BEFORE the session is created, so a signed-in person is always one
  // the Login accounts screen can see has signed in. Wrapped: a failed write
  // must never cost somebody their login — the stamp is for a report, the
  // session is their job.
  try {
    db.prepare("UPDATE profiles SET last_login_at = ? WHERE id = ?").run(new Date().toISOString(), profile.id);
  } catch (err) {
    console.error("[auth] could not stamp last_login_at:", err?.message || err);
  }
...
export async function register(prevState, formData) {
  ...
  // Registering IS signing in, so it stamps too. Without this a
  // self-registered account would read as "never signed in" on the Login
  // accounts screen until their second visit, and get chased for it.
  try {
    db.prepare("UPDATE profiles SET last_login_at = ? WHERE id = ?").run(new Date().toISOString(), id);
  } catch (err) {
    console.error("[auth] could not stamp last_login_at on register:", err?.message || err);
  }
