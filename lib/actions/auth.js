...
import { isProvider } from "@/lib/auth/oauth";
...
export async function register(prevState, formData) {
  ...
  // Came here from "Continue with Google/Microsoft/Facebook" with no account
  // yet? The identity waited in a signed cookie; attach it now, but only if
  // the address they registered is the one the provider vouched for.
  await attachPendingIdentity(db, id, email, now);
...
async function attachPendingIdentity(db, profileId, email, now) {
  const store = await cookies();
  const pending = await decrypt(store.get(PENDING_COOKIE)?.value);
  store.delete(PENDING_COOKIE);
  if (!pending || !isProvider(pending.p) || !pending.sub) return;
  if (String(pending.e || "").toLowerCase() !== email) return;
  const taken = db.prepare("SELECT 1 FROM auth_identities WHERE provider = ? AND provider_subject = ?").get(pending.p, pending.sub);
  if (taken) return;
  db.prepare(
    "INSERT INTO auth_identities (id, profile_id, provider, provider_subject, email, created_at) VALUES (?, ?, ?, ?, ?, ?)"
  ).run(randomUUID(), profileId, pending.p, pending.sub, email, now);
}

/** Profile page: forget a linked provider. The password always remains. */
export async function unlinkIdentity(formData) {
  const session = await requireSession();
  const provider = String(formData.get("provider") || "");
  if (!isProvider(provider)) return { error: "Unknown provider." };
  getDb().prepare("DELETE FROM auth_identities WHERE profile_id = ? AND provider = ?").run(session.userId, provider);
  revalidatePath("/profile");
  redirect("/profile?unlinked=" + provider);
}
