...
/**
 * Can the mushaf glyphs be used at all in this Arabic script mode?
 *
 * Tajweed and makhraj colour LETTERS — they take the Uthmani text apart and
 * tint spans of it by rule. A mushaf glyph is a whole WORD in a single
 * private-use codepoint, so there is no letter inside it to colour and no way
 * to split it. The two cannot both be on.
 *
 * The colouring wins. A teacher who switched tajweed on asked to see the
 * rules, and the reflowed text shows them correctly; silently showing an
 * uncoloured page instead answers a question they did not ask.
 */
export function glyphsAllowedFor(displayMode) {
