...
export function createQuranStore({ dataSource, bookmarkService, readingLogService, defaults = {} }) {
  let state = {
    ...
    pageVerses: [],
    pageStatus: "loading",
    // Draw a page in the mushaf's own per-page font when one can be fetched.
    // On by default because it is the whole point of page mode; a teacher on a
    // slow connection can turn it off and still read the page.
    mushafFont: true,
    // -------------------------------------------------------------------------
  ...
  function hydrate() {
    ...
    try {
      ...
      if (typeof p.mushafFont === "boolean") patch.mushafFont = p.mushafFont;
  ...
  return {
    ...
    toggleMushafFont() {
      setState({ mushafFont: !state.mushafFont });
      persist();
    },
    nextPage() {
