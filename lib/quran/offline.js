...
const pageKey = (pageNumber, translationId) => `page:${pageNumber}:${translationId}`;
...
export function withOfflineCache(source) {
  return {
    ...
    // Mushaf pages cache the same way, so a page read once is readable offline.
    // "Save for offline" deliberately does NOT walk all 604 — it already saves
    // every surah, which is the same text, and doubling a download a teacher
    // waits through to pre-fetch a second cut of it is not a fair trade.
    getPage: (pageNumber, translationId) =>
      netFirst(pageKey(pageNumber, translationId), () => source.getPage(pageNumber, translationId)),
