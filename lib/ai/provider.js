...
export function normalisePack(input, analysis) {
  ...
  return {
    ...
    // Approved teacher proposals of kind 'note' (lib/actions/packs.js). Not
    // from the model; shown on the pack page as "From teachers".
    teacherNotes: arr(obj.teacherNotes).map(str).filter(Boolean),
    uncertain,
  };
}
