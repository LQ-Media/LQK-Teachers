...
function ensureSchema(database) {
  ...
  database.exec(`
    ...
    -- ── Event invites ────────────────────────────────────────────────────
    -- A parent contact list imported from CSV, kept so the same cohort can be
    -- invited again without re-uploading. Contacts here are PARENTS, not the
    -- students in the students table — the roster carries no contact details.
    CREATE TABLE IF NOT EXISTS contact_lists (
      id TEXT PRIMARY KEY,
      name TEXT NOT NULL,
      source_name TEXT,              -- original CSV filename, for recognising it later
      created_by TEXT REFERENCES profiles(id),
      created_at TEXT NOT NULL,
      updated_at TEXT NOT NULL
    );
    ...
    CREATE TABLE IF NOT EXISTS contacts (
      ...
      name TEXT,
      email TEXT,
      phone TEXT,                    -- E.164 digits, no leading '+' (WATI's format)
      child_name TEXT,
      created_at TEXT NOT NULL
    );
    CREATE INDEX IF NOT EXISTS idx_contacts_list ON contacts(list_id);

    -- One invite. design and agenda are JSON blobs so the builder can grow
    -- new styling knobs without a migration; everything the invite page reads
    -- by name lives in its own column.
    CREATE TABLE IF NOT EXISTS events (
      id TEXT PRIMARY KEY,
      slug TEXT NOT NULL UNIQUE,     -- public URL segment: /invite/<slug>
      status TEXT NOT NULL DEFAULT 'draft' CHECK (status IN ('draft', 'sent')),
      language TEXT NOT NULL DEFAULT 'en'
        CHECK (language IN ('en', 'ms', 'zh', 'ta', 'ar')),
      title TEXT NOT NULL,
      tagline TEXT,
      body TEXT,                     -- free-text description paragraph
      starts_at TEXT,                -- 'YYYY-MM-DDTHH:MM' in Singapore local time
      ends_at TEXT,
      venue_name TEXT,
      venue_address TEXT,
      map_url TEXT,
      ...
      dress_code TEXT,
      bring TEXT,
      agenda TEXT,                   -- JSON: [{ "time": "09:00", "activity": "..." }]
      host_name TEXT,
      contact_line TEXT,             -- "Questions? WhatsApp Ustazah Aisyah on ..."
      logo_path TEXT,                -- filename under uploads/events/
      background_path TEXT,
      design TEXT,                   -- JSON: colours, fonts, alignment, overlay
      created_by TEXT REFERENCES profiles(id),
      created_at TEXT NOT NULL,
      updated_at TEXT NOT NULL,
      sent_at TEXT
    ...
    -- One row per parent per event: who was invited, whether each channel
    -- reached them, and what they answered. Snapshotted from the contact list
    -- at send time, so later edits to the list never rewrite a sent invite.
    CREATE TABLE IF NOT EXISTS event_recipients (
      id TEXT PRIMARY KEY,
      event_id TEXT NOT NULL REFERENCES events(id) ON DELETE CASCADE,
      token TEXT NOT NULL UNIQUE,    -- unguessable per-parent RSVP key
      name TEXT,
      email TEXT,
      phone TEXT,
      child_name TEXT,
...
export function getDb() {
  ...
  // Write-ahead logging: readers no longer block on a writer, and a writer no
  // longer blocks readers. Without it SQLite takes a whole-database lock for
  // every write, so a long-running job — an event invite going out to a
  // thousand parents writes a delivery status per parent — makes concurrent
  // requests fail outright with SQLITE_BUSY. Observed exactly that while
  // load-testing the invite sender.
  //
  // busy_timeout covers the remaining case: when two writers do collide, wait
  // up to five seconds for the lock instead of erroring immediately.
  // Both are per-connection except journal_mode, which is stored in the file.
