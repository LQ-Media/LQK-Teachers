...
import { t, fontEmail } from "./design.js";
...
export function renderInviteEmail(event, { inviteUrl, assetBase = "", recipientName = "" } = {}) {
  ...
  const headingFont = fontEmail(d.headingFont);
  const bodyFont = fontEmail(d.bodyFont);
  const align = d.align;
  ...
  const agendaRows = (event.agenda || [])
    ...
    .map(
      (a) => `
      <tr>
        <td style="padding:0 12px 8px 0;white-space:nowrap;vertical-align:top;font-size:14px;color:${d.accentColor};font-weight:bold;">${escapeHtml(a.time)}</td>
  ...
  return `<!doctype html>
<html lang="${event.language === "ms" ? "ms" : "en"}">
<head>
<meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>${escapeHtml(event.title)}</title>
</head>
<body style="margin:0;padding:0;background:${d.pageColor};font-family:${bodyFont};-webkit-text-size-adjust:100%;">
          ...
          ${
            agendaRows
              ? `<tr><td style="padding:6px 32px 0 32px;">
                   ...
                   <table role="presentation" cellpadding="0" cellspacing="0" border="0">${agendaRows}</table>
                 </td></tr>`
              : ""
          }
