  ...
  return plainAll(
    getDb()
      ...
      `SELECT g.*, r.attending, r.adults, r.children, r.message,
              ...
              r.photo_drive_id, r.checked_in_at, r.custom_json, r.attendees_json,
...
export function saveRsvp(guestId, eventId, data) {
  ...
  const row = {
    ...
    // The family photo is uploaded to Drive the moment it is picked, which on a
    // first visit is BEFORE this row exists — so the id travels back with the
    // reply and is written here. NULL means "the guest sent none this time",
    // never "clear the one already recorded": a family editing their headcount
    // must not unlink a photo Karim already has.
    photo_drive_id: data.photoDriveId || null,
  ...
  if (existing) {
    db.prepare(
      `UPDATE event_rsvps SET attending = ?, adults = ?, children = ?, extra_names = ?,
         decline_reason = ?, decline_reason_note = ?, message = ?, photo_consent = ?,
         photo_drive_id = COALESCE(?, photo_drive_id),
         custom_json = ?, attendees_json = ?, updated_at = ?
       WHERE guest_id = ?`
    ).run(
      row.attending, row.adults, row.children, row.extra_names,
      row.decline_reason, row.decline_reason_note, row.message, row.photo_consent,
      row.photo_drive_id, row.custom_json, row.attendees_json, ts, guestId
    );
  } else {
    db.prepare(
      `INSERT INTO event_rsvps (guest_id, event_id, attending, adults, children,
         extra_names, decline_reason, decline_reason_note, message, photo_consent,
         photo_drive_id, custom_json, attendees_json, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    ).run(
      guestId, eventId, row.attending, row.adults, row.children, row.extra_names,
      row.decline_reason, row.decline_reason_note, row.message, row.photo_consent,
      row.photo_drive_id, row.custom_json, row.attendees_json, ts, ts
    );
  }
...
export function eventStats(eventId) {
  ...
  const base = db
    ...
    .get(eventId, eventId, eventId, eventId, eventId, eventId, eventId, eventId, eventId);
  return {
    ...base,
