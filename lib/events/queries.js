...
export function saveRsvp(guestId, eventId, data) {
  ...
  const row = {
    attending,
    adults: attending === "yes" ? Math.max(0, Math.min(20, Number(data.adults) || 0)) : 0,
    children: attending === "yes" ? Math.max(0, Math.min(20, Number(data.children) || 0)) : 0,
  ...
  if (existing) {
    db.prepare(
      ...
      row.decline_reason, row.decline_reason_note, row.message, row.photo_consent, ts, guestId
    );
  } else {
    db.prepare(
      ...
      row.decline_reason, row.decline_reason_note, row.message, row.photo_consent, ts, ts
    );
  }
...
export function markContributionPending(guestId, title) {
  getDb()
    .prepare(
      `UPDATE event_rsvps
...
export function guestListCsv(eventId) {
  const rows = getDb()
    .prepare(
      `SELECT g.name, g.family_name, r.attending, r.adults, r.children,
  ...
  const body = rows.map((r) =>
    [
      ...
      r.message,
      r.extra_names,
      r.attending !== "yes" ? "" : r.contribution_status === "paid" ? "Paid" : "Unpaid",
