...
/* The host's free-text body, split into paragraphs.

   Lives here rather than in a component because THREE renderers must agree on
   what a paragraph is — the invitation card, the HTML email and the plain-text
   email part. A blank line starts a new paragraph; a single newline stays a
   line break inside one. Control characters and bidi overrides are stripped for
   the same reason as field labels: this is admin text on a guest's screen.

   Capped rather than unbounded — an invitation is not a newsletter, and an
   accidental paste of a whole chat log should not become the email. */
export const MAX_BODY_TEXT = 4000;

/* Deliberately does NOT use cleanText(): that strips \u0000-\u001F, which
   includes the newlines this function exists to read. Tab (\u0009) and
   newline (\u000A) both survive — stripping a tab glues two words together
   — and every other control character and bidi override does not. */
export function bodyParagraphs(text) {
