...
export function whatsappParameters(event, recipient) {
  return [
    { name: "1", value: recipient.name || (event.language === "ms" ? "Ibu/Bapa" : "there") },
...
async function sendWhatsAppTo(event, recipient) {
  await sendTemplateMessage(recipient.phone, whatsappParameters(event, recipient), `LQK ${event.title}`.slice(0, 60));
}
