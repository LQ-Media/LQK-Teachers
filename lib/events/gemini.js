...
const SPEC_MODEL = process.env.GEMINI_TEXT_MODEL || "gemini-flash-latest";
...
async function callGemini(model, body, { timeoutMs = 90_000 } = {}) {
  const key = process.env.GEMINI_API_KEY;
  ...
  if (candidate.finishReason === "MAX_TOKENS") {
    return { ok: false, error: "Gemini ran out of output budget before finishing the design." };
  }
