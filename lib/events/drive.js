...
export async function uploadPhoto({ base64, mime, filename, eventSlug }) {
  return uploadFile({ base64, mime: mime || "image/jpeg", filename, subfolder: eventSlug || null });
}

/* The general form. `folderId` defaults to the events folder; the assessment
   evidence route passes LQK_ASSESS_DRIVE_FOLDER_ID so recordings never land
   among guests' family photos. `subfolder` may be a path ("2026/Siti Aminah"):
   the script creates each level on demand (see scripts/drive-upload.gs). */
export async function uploadFile({ base64, mime, filename, folderId, subfolder }) {
  ...
  try {
    const res = await fetch(process.env.LQK_DRIVE_URL, {
      ...
      body: JSON.stringify({
        ...
        folderId: folderId || process.env.LQK_DRIVE_FOLDER_ID || null,
