  ...
  getDb()
    .prepare(
      `UPDATE qr_events SET title = ?, venue_name = ?, starts_at = ?, status = ?, intro = ?,
         fields_json = ?, photo_enabled = ?, photo_prompt = ?, photo_blurb = ?,
         photo_consent_text = ?, updated_at = ? WHERE id = ?`,
    )
    .run(
      config.title,
...
export function checkIn(eventId, input) {
  ...
  try {
    db.prepare(
      `INSERT INTO qr_registrations (id, qr_event_id, token, family_name, contact_name, phone, email, answers_json, created_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    ).run(
      id,
      eventId,
...
export function listRegistrations(eventId) {
  ...
  const rows = plainAll(
    db
      .prepare(
        `SELECT r.*,
                (SELECT COUNT(*) FROM qr_attendance a WHERE a.registration_id = r.id) AS head_count,
...
export function listAttendance(eventId) {
  return plainAll(
    getDb()
      .prepare(
        `SELECT a.*, r.family_name, r.contact_name, r.phone, r.email, r.answers_json, r.token
...
export function qrStats(eventId) {
  ...
  const people = one("SELECT COUNT(*) AS n FROM qr_attendance WHERE qr_event_id = ?", eventId);
  const adults = one(
