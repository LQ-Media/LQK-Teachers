...
export function savePhoto(eventId, registrationId, { fileName, mime, bytes, prompt, consentText }) {
  ...
  db.prepare(
    `INSERT INTO qr_photos (id, qr_event_id, registration_id, status, file_name, mime, bytes,
     ...
     VALUES (?, ?, ?, 'ready', ?, ?, ?, ?, ?, ?, ?)`,
  ).run(id, eventId, registrationId, fileName, mime, bytes, prompt, consentText, ts, ts);

  if (previous?.file_name && previous.file_name !== fileName) removePhoto(previous.file_name);

  /* Returned so every <img> can carry it as a cache-buster. The photo route
     is served with a long max-age (these are 1–2 MB), and the URL used to be
     the same string every time — so regenerating after changing the prompt
     showed the family the OLD picture out of the browser cache, and looked
     exactly like the new prompt had been ignored. */
  return id;
}

/* Every finished picture for an event, newest first, with the family it
   belongs to. Feeds the gallery and the slideshow. */
export function listPhotos(eventId) {
  return plainAll(
    getDb()
      .prepare(
        `SELECT p.id, p.registration_id, p.created_at, p.mime, p.bytes,
                r.token, r.family_name, r.contact_name
           FROM qr_photos p
           JOIN qr_registrations r ON r.id = p.registration_id
          WHERE p.qr_event_id = ? AND p.status = 'ready' AND p.file_name IS NOT NULL
