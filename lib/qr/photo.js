...
export function buildPrompt(event, assets) {
  ...
  const shape = isAspect(event?.photo_aspect) ? event.photo_aspect : DEFAULT_ASPECT;

  return [
    "You are producing a single finished event photograph for a family to keep.",
    occasion ? `The occasion is ${occasion}.` : "",
    `The finished picture is ${shape}. Compose for that shape — do not leave the people cropped at the edge.`,
    "",
...
export async function composeFamilyPhoto({ event, assets, familyPhoto }) {
  ...
  for (const asset of assets) {
    ...
    if (!bytes) continue;
    /* Karim's own label for this image is appended to the standard line. With
       two event logos in the pile, "the EVENT LOGO" twice tells the model
       nothing; "the EVENT LOGO (Maulid 2026 wordmark)" tells it which is
       which, and lets his prompt refer to them by name. */
    const named = asset.label ? ` It is: ${asset.label}.` : "";
    parts.push({ text: `\n${ASSET_INTRO[asset.kind] || ASSET_INTRO.extra}${named}` });
  ...
  const res = await callGemini(
    IMAGE_MODEL,
    {
      ...
      generationConfig: {
        ...
        imageConfig: { aspectRatio: isAspect(event?.photo_aspect) ? event.photo_aspect : DEFAULT_ASPECT },
      },
    },
