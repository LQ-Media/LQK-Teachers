...
export const QR_SCHEMA_SQL = `
  CREATE TABLE IF NOT EXISTS qr_events (
    ...
    -- Optional headcount question. Some events only need a number for the
    -- caterer, and naming a grandmother, two aunties and a neighbour's child
    -- is more typing than a parent will do standing in a doorway.
    ask_pax INTEGER NOT NULL DEFAULT 0,
    pax_label TEXT,               -- his own wording for the question
    ...
    photo_aspect TEXT,            -- the shape of the generated picture
  ...
  CREATE TABLE IF NOT EXISTS qr_registrations (
    ...
    -- People in the party who were counted but not named. ADDITIONAL to the
    -- qr_attendance rows, never overlapping them, so a headcount is always
    -- named + this and never has to guess which it is.
