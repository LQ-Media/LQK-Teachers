...
// startOfWeek only — calendar.js is pure and imports nothing but rates.js, so
// there is no cycle, and "every 2 weeks" has to mean the same fortnight here
// as the grid draws.
 ...
 */
export function expandDates(fromDate, toDate, weekdays, skipDates, everyWeeks = 1) {
  const want = new Set(weekdays || []);
  const skip = skipDates || new Set();
  const out = [];
  if (!want.size || !fromDate || !toDate || fromDate > toDate) return out;

  // "Every N weeks", from Sling's REPEAT menu. Counted from the week FROMDATE
  // falls in, so "every 2 weeks" starting on a Thursday means that Thursday and
  // then a fortnight later — not "even-numbered weeks of the year", which is
  // the same thing half the time and baffling the other half.
  const step = Number.isInteger(everyWeeks) && everyWeeks > 0 ? everyWeeks : 1;
  // Counted in CALENDAR weeks from the Monday of fromDate's week, not in
  // seven-day blocks from fromDate itself. Starting on a Wednesday with
  // "Mon + Wed, every 2 weeks", blocks would put that week's Wednesday and the
  // NEXT Monday in the same fortnight and then skip a Monday — the grid would
  // show a pattern nobody asked for. Calendar weeks give Mon+Wed together,
  // which is what the menu says and what Sling does.
  const anchor = startOfWeek(fromDate);
  const weekOf = (d) => Math.floor(daysBetweenSg(anchor, d) / 7);

  // Guard against a typo'd end date generating years of rows.
  for (let d = fromDate, i = 0; d <= toDate && i < 800; d = addSgDays(d, 1), i++) {
    if (!want.has(sgWeekday(d)) || skip.has(d)) continue;
    if (step > 1 && weekOf(d) % step !== 0) continue;
    out.push(d);
  }
  return out;
}
...
/**
 * Whole days from `a` to `b`, both Singapore dates.
 *
 * Midday, so no offset or rounding can make a day boundary land on the wrong
 * side — the same reason lib/hours/calendar.js does it.
 */
export function daysBetweenSg(a, b) {
  const ms = new Date(`${b}T12:00:00+08:00`) - new Date(`${a}T12:00:00+08:00`);
...
export function buildShiftRows({
  ...
  const dates = expandDates(fromDate, toDate, weekdays, skipSet, everyWeeks);
  const rows = [];
  ...
  const skipped = skipHolidays
    ? expandDates(fromDate, toDate, weekdays, new Set(), everyWeeks)
