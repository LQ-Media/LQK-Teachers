...
export function roleOf(shift) {
  ...
  // A colour chosen on the Positions screen wins over anything derived, because
  // somebody chose it on purpose. Cancelled still beats it: the question a grey
  // block answers is "is this happening?", and no colour choice changes that.
  if (shift.positionColour && ROLE_COLOURS[shift.positionColour]) return shift.positionColour;
