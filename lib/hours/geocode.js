 ...
 *
 * Returns null on anything unexpected rather than throwing — a caller that
 * cannot resolve a centre must leave it unresolved, not crash a sync or, worse,
 * write a coordinate it is not sure about.
 *
...
export async function syncLocationCoords({ force = false } = {}) {
  ...
  for (const loc of LOCATIONS) {
    ...
    if (!hit) {
      report.push({ key: loc.key, label: loc.label, status: "failed", postalCode: loc.postalCode });
      continue;
    }
    saveCoords(loc.key, hit.lat, hit.lng, loc.postalCode, "onemap");
    report.push({ key: loc.key, label: loc.label, status: "resolved", lat: hit.lat, lng: hit.lng });
  }
  return report;
}
