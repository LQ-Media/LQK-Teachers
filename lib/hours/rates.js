...
/**
 * Hours as LQK rounds them for payroll: under 30 minutes past the hour rounds
 * down, over 30 rounds up. Karim's rule of 16 Sep 2026, his own examples being
 * 5h20m -> 5 and 5h45m -> 6.
 *
 * Exactly 30 minutes rounds UP. He did not say, and every other boundary in
 * this file favours the teacher, so this one does too. It is one line to flip
 * if that is wrong.
 *
 * This is applied ONCE, to a monthly total, never per shift — rounding each
 * shift and then adding them would compound the error across ~20 shifts into
 * something nobody could reconcile against the roster.
 */
export function payrollHours(minutes) {
  const m = Math.max(0, Math.round(minutes));
  return Math.floor(m / 60) + (m % 60 >= 30 ? 1 : 0);
}
