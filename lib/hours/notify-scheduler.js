    ...
    .prepare(
      `SELECT s.id, s.teacher_id, s.category, s.status, s.branch,
       ...
       WHERE s.starts_at BETWEEN ? AND ?
         ...
         -- A draft sends nothing. "Teachers can't see it and no reminder is
         -- sent" is what the PUBLISH toggle says on the form, and a reminder
         -- for a shift they cannot see is worse than no reminder at all.
         AND s.published = 1`
    )
    .all(from, to)
    .map((r) => ({
