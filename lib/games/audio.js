 ...
 * the request entirely. 0.25x and 0.1x are here because they were asked for
 * and a child struggling with a makhraj may genuinely need them; do not be
 * surprised if a given tablet treats them as the same as 0.5x, and judge them
 * by ear on the device rather than by the number.
 */
export const SAY_RATES = [1, 0.8, 0.65, 0.5, 0.25, 0.1];
...
function pickArabicVoice(synth) {
  ...
  if (!arabicVoice || !voices.includes(arabicVoice)) {
    ...
    // A LOCAL voice in preference to a network one. Chrome's Arabic voice is
    // usually "Google العربية", which is synthesised on Google's servers: it
    // needs connectivity, it is rate-limited, and when it fails it fails
    // silently — which is most of why Chrome goes quiet after the first
    // letter while iOS (all-local voices) does not. In a classroom on poor
    // wifi the local voice is the one that keeps working.
    arabicVoice = arabic.find((v) => v.localService) ?? arabic[0] ?? null;
  }
  return arabicVoice;
}
...
function speakAttempt(synth, voice, text, rate, attempt) {
  ...
  u.voice = voice;
  u.lang = voice.lang;
  // Well under conversational pace by default: this is a sound being modelled
  // for a child to copy, not a sentence being read. The teacher can change it.
  u.rate = rate;
...
export function speakArabic(text, { rate = getSayRate() } = {}) {
