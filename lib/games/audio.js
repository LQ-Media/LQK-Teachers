 ...
 */
export const SAY_RATE = 0.65;
export const CLIP_RATE = 1;

function playBuffer(buf, { gain = 1, rate = CLIP_RATE } = {}) {
  const c = unlockAudio();
...
export function speakArabic(text, { rate = SAY_RATE } = {}) {
  ...
  u.voice = voice;
  u.lang = voice.lang;
  // Well under conversational pace: this is a sound being modelled for a
  // child to copy, not a sentence being read. See SAY_RATE.
  u.rate = rate;
  u.onend = () => {
    speaking = null;
  };
