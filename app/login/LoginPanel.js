...
/* The signed-out panel. `providers` and `notice` are resolved on the server in
   page.js — this component only decides which tab is showing. */
export default function LoginPanel({ providers = [], notice = "" }) {
  ...
  return (
    <div className="min-h-screen bg-paper flex items-center justify-center p-6">
      <div className="w-full max-w-sm">
        ...
        {notice && (
          <p className="mb-4 rounded-control bg-rust-soft px-3 py-2 text-[12px] font-medium leading-relaxed text-rust">
            {notice}
          </p>
        )}
        ...
        {/* Social sign-in matches an EXISTING account only, so it belongs to the
            sign-in tab. Offering it under "Register" would promise an account it
            deliberately can't create (see the callback route). */}
        {mode === "signin" && <SocialSignIn providers={providers} />}
    ...
    <>
        ...
        <form action={formAction} className="bg-white border-[0.5px] border-line rounded-card p-6 flex flex-col gap-4">
          ...
          <Link
            ...
            className="text-center text-[12px] font-semibold text-charcoal-soft hover:text-charcoal"
          >
