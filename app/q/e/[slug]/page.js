...
import { getPublicQrEvent, getFamilyByToken } from "@/lib/events/passport-queries";
import { displayName, PASS_COOKIE } from "@/lib/events/passport";
...
export default async function CheckInPage({ params, searchParams }) {
  ...
  const alreadyHere = existing && existing.family.event_id === event.id ? existing.family : null;
  ...
  return (
    <main className="mx-auto min-h-dvh w-full max-w-lg px-5 py-10">
      <header className="text-center">
        ...
        {event.venue_name ? (
          <p className="mt-1 text-sm text-charcoal-soft">{event.venue_name}</p>
        ) : null}
      </header>
