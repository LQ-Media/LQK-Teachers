...
/* The door poster.
   ...
   only the things families and volunteers actually open.
...
export async function generateMetadata({ params }) {
  ...
  return { title: `Poster · ${event?.title || "Registration"}` };
}
  ...
  return (
    <div className="px-4 py-6 sm:p-8">
      <div className="mx-auto max-w-2xl print:hidden">
        <Link href={`/qr/${id}`} className="mb-4 inline-block text-sm text-gold hover:underline">
          ← {event.title}
        </Link>
        {isLocal ? (
      ...
      <article className="mx-auto mt-6 max-w-2xl rounded-card border border-line bg-white p-10 text-center print:mt-0 print:border-0 print:p-0">
        ...
        <p className="mx-auto mt-6 max-w-md text-xl leading-relaxed text-charcoal-soft">
          Scan to find your name and start collecting letters around the hall.
        </p>
