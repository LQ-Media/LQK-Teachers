    ...
    <>
      ...
      >
        ...
        <div style={{ position: "fixed", left: -9999, top: 0 }} aria-hidden="true">
          ...
          >
            <div style={{ fontSize: 12, letterSpacing: 2, color: "#96681A", fontWeight: 700 }}>LITTLE QURAN KIDS</div>
            <div style={{ fontSize: 22, fontWeight: 700, marginTop: 6 }}>{titleCase(student.name)}</div>
            <div style={{ fontSize: 13, color: "#776E5D", marginTop: 2 }}>{sgTodayLabel()}</div>
            <div style={{ height: 1, background: "#E8DDC5", margin: "16px 0" }} />
            <div style={{ fontSize: 12, color: "#776E5D" }}>Today’s portion</div>
            <div style={{ fontSize: 20, fontWeight: 700, marginTop: 2 }}>{label}</div>
            {grade && (
              <div style={{ marginTop: 12, display: "inline-block", background: "#F8E3BE", color: "#6E5320", borderRadius: 999, padding: "4px 14px", fontSize: 14, fontWeight: 700 }}>
                {grade}
              </div>
            )}
            {note && <div style={{ marginTop: 14, fontSize: 14, color: "#3B372B" }}>{note}</div>}
            <div style={{ marginTop: 18, fontSize: 11, color: "#776E5D" }}>Teacher: {teacherName}</div>
          </div>
        </div>
      </div>
    </>
