/**
 * The LQK monogram — L, Q (crescent + star inside the counter), K.
 *
 * The paths are lifted verbatim from brand/lqk-logo.svg, the artwork Karim
 * supplied, and BRAND_ORANGE is that file’s own fill. This is the ONLY
 * definition of the mark in the app: the favicon, the PWA icons and the Apple
 * touch icon are all rendered from the same SVG by scripts/generate-icons.mjs.
 *
 * Never re-tint it. The mark spent a while in rose #DE7E9F to chase a theme;
 * a logo does not follow the palette, the palette follows the logo.
 */
export const BRAND_ORANGE = "#F0A41F";

export default function LqkMark({ className, title }) {
  return (
    <svg
      className={className}
